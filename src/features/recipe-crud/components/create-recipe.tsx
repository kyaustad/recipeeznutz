...
import type { NewRecipe } from "@/bindings/NewRecipe";
import { z } from "zod";
...
function toNewRecipe(values: RecipeFormValues): NewRecipe {
  return {
...
export function AddRecipeDialog({
  ...
  const onSubmit = handleSubmit(async (values) => {
    const recipe = toNewRecipe(values);
    setSubmitting(true);
    try {
      await invoke("create_recipe_command", { recipe });
  ...
  return (
    <Dialog
      ...
      modal={true}
    >
      <DialogContent className="flex max-h-[70vh] min-h-[50vh] min-w-[50vw] flex-col gap-0 overflow-hidden">
        ...
        >
          <FieldGroup className="gap-4">
            ...
            <Field>
              <FieldLabel htmlFor="recipe-notes">Notes (optional)</FieldLabel>
              <FieldContent>
                <Textarea
                  id="recipe-notes"
